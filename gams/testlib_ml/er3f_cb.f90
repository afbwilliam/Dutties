!    er3f_cb.f90
!  Fortran implementation of the external functions for error example 3
!  in the file er3.gms.
...
!           something is wrong
            call MSG (MSGCB, 3, ' ** fIndex has unexpected value.')
...
!           strange. The value of dfdx(i) could be inherited from the
!           previous equation that defined dfdx(i).
!               dfdx(i)    = -0.5d0*x(6+i1)
               dfdx(i1)   = +0.5d0*x(6+i)
               dfdx(6+i)  = +0.5d0*x(i1)
               dfdx(6+i1) = -0.5d0*x(i)
...
!        Unknown Mode:
         call MSG (MSGCB, 3, ' ** Unknown Mode value.')
         result = 2             ! error in call
      endif
