! er1f_cb.f90
!  Fortran implementation of the external functions for error example 1
!  in the file er1.gms.
!  Look for **ERROR** to find the mistake and compare to the GAMS
!  output.
...
FUNCTION gefuncx (cbType, icntr, x, f, dfdx, msgcb, usrMem) RESULT (result)
  ...
  IF (icntr(I_Mode) == 1) THEN
     ...
     CALL msg2 (cbType, usrMem, msgcb, 3, '**** Using External Function in er1f_cb.f90.')
...
!
!  **ERROR** We forget the constant derivative
!
!           dfdx(NI+1) = -1.d0
           DO i = 1, NI
              dfdx(i) = 0.d0
