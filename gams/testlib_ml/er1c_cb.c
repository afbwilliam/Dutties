/*
  ...
  C implementation of the external functions for error example 1
  in the file er1.gms.
  Look for **ERROR** and compare with the message in GAMS.
  ...
  */
  ...
  if ( icntr[I_Mode] == DOINIT ) {
    ...
    */
    ...
    MSGCB (LOGFILE | STAFILE,"--- GEFUNC in er1c_cb.c is being initialized.");
    ...
    */
    if ( findex == 1 ) {
      ...
      */
      if ( dodrv ) {
/* **ERROR** Do not define the constant derivative
        d[ni] = -1.0;  */
        for (i = 0; i < ni; i++) {
          d[i] = 0;
          for (j = 0; j < ni; j++ )
