...
public:
  ...
  inline void     GetMessage (int NrMsg, std::string &info, int &iType)
  { optGetMessage (optHandle_, NrMsg, tmpS0, &iType); info=tmpS0; return; }
  ...
  { optVersion (optHandle_, tmpS0); sversion=tmpS0; return; }
  inline void     DefinitionFile (std::string &sfilename)
  { optDefinitionFile (optHandle_, tmpS0); sfilename=tmpS0; return; }
  ...
  inline int     GetSynonym (int NrSyn, std::string &SSyn, std::string &SName)
  { int rc=optGetSynonym (optHandle_, NrSyn, tmpS0, tmpS1); SSyn=tmpS0; SName=tmpS1; return rc; }
  inline void     EchoSet (int AIVal)
  {  return optEchoSet (optHandle_, AIVal);}
  inline int     EOLOnlySet (int AIVal)
  {  return optEOLOnlySet (optHandle_, AIVal);}
  inline void     NoBoundsSet (int AIVal)
  {  return optNoBoundsSet (optHandle_, AIVal);}
  ...
  inline int     GetDotOptNr (int ANr, std::string &VEName, int &AObjNr, int &ADim, double &AValue)
  { int rc=optGetDotOptNr (optHandle_, ANr, tmpS0, &AObjNr, &ADim, &AValue); VEName=tmpS0; return rc; }
  ...
  inline int     GetVarEquMapNr (int maptype, int ANr, std::string &EquName, std::string &VarName, int &EquDim, int &VarDim, int &AValue)
  { int rc=optGetVarEquMapNr (optHandle_, maptype, ANr, tmpS0, tmpS1, &EquDim, &VarDim, &AValue); EquName=tmpS0; VarName=tmpS1; return rc; }
  inline int     GetEquVarEquMapNr (int maptype, int ANr, int ADim, std::string &AIndex)
  { int rc=optGetEquVarEquMapNr (optHandle_, maptype, ANr, ADim, tmpS0); AIndex=tmpS0; return rc; }
  inline int     GetVarVarEquMapNr (int maptype, int ANr, int ADim, std::string &AIndex)
  { int rc=optGetVarVarEquMapNr (optHandle_, maptype, ANr, ADim, tmpS0); AIndex=tmpS0; return rc; }
  inline int     VarEquMapCount (int maptype, int &ANrErrors)
  {  return optVarEquMapCount (optHandle_, maptype, &ANrErrors);}
  ...
  inline int     SetRefNr (int ANr, int ARefNr)
  {  return optSetRefNr (optHandle_, ANr, ARefNr);}
  inline int     SetRefNrStr (const std::string &AOpt, int ARefNr)
  {  return optSetRefNrStr (optHandle_, AOpt.c_str(), ARefNr);}
  ...
  inline int     GetNameOpt (const std::string &ASVal, std::string &solver, int &opt)
  { int rc=optGetNameOpt (optHandle_, ASVal.c_str(), tmpS0, &opt); solver=tmpS0; return rc; }
  inline bool     GetDefinedStr (const std::string &AName)
  {  return optGetDefinedStr (optHandle_, AName.c_str()) != 0;}
  inline int     GetIntStr (const std::string &AName)
  {  return optGetIntStr (optHandle_, AName.c_str());}
  inline double     GetDblStr (const std::string &AName)
  {  return optGetDblStr (optHandle_, AName.c_str());}
  inline std::string     GetStrStr (const std::string &AName)
  { char *rc=optGetStrStr (optHandle_, AName.c_str(), tmpS0); return (std::string)rc; }
  inline void     SetIntStr (const std::string &AName, int AIVal)
  {  return optSetIntStr (optHandle_, AName.c_str(), AIVal);}
  inline void     SetDblStr (const std::string &AName, double ADVal)
  {  return optSetDblStr (optHandle_, AName.c_str(), ADVal);}
  inline void     SetStrStr (const std::string &AName, const std::string &ASVal)
  {  return optSetStrStr (optHandle_, AName.c_str(), ASVal.c_str());}
  ...
  inline void    RecentEnabledSet(const int  x)  { optRecentEnabledSet (optHandle_, x); }
