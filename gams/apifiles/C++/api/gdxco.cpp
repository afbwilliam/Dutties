...
{
  char cmsg[257];
  int initresult = gdxCreate (&gdxHandle_, cmsg, sizeof(cmsg));
  msg = cmsg;
  extHandle = 0;
  return initresult;
} // Init(std::string &msg)
...
{
  char cmsg[257];
  int initresult = gdxCreateD (&gdxHandle_, dirname.c_str(), cmsg, sizeof(cmsg));
  msg = cmsg;
  extHandle = 0;
  return initresult;
} // Init(const std::string &dirname; std::string &msg)
...
Init (const std::string &dirname, const std::string &libname, std::string &msg)
{
  ...
  int initresult = gdxCreateL (&gdxHandle_, lib.c_str(), cmsg, sizeof(cmsg));
  msg = cmsg;
  extHandle = 0;
  return initresult;
} // Init(const std::string &dirname; const std::string &libname; std::string &msg)
