...
{
  char cmsg[257];
  int initresult = optCreate (&optHandle_, cmsg, sizeof(cmsg));
  msg = cmsg;
  extHandle = 0;
  return initresult;
} // Init(std::string &msg)
...
{
  char cmsg[257];
  int initresult = optCreateD (&optHandle_, dirname.c_str(), cmsg, sizeof(cmsg));
  msg = cmsg;
  extHandle = 0;
  return initresult;
} // Init(const std::string &dirname; std::string &msg)
...
Init (const std::string &dirname, const std::string &libname, std::string &msg)
{
  ...
  int initresult = optCreateL (&optHandle_, lib.c_str(), cmsg, sizeof(cmsg));
  msg = cmsg;
  extHandle = 0;
  return initresult;
} // Init(const std::string &dirname; const std::string &libname; std::string &msg)
