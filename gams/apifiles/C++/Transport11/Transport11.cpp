// Transport11.cpp : main project file.
...
static void CreateSaveRestart(String^ cpFileName, array<String^>^ args)
{
	GAMSWorkspace^ ws;
	if (args->Length > 0) 
		ws = gcnew GAMSWorkspace(Path::GetDirectoryName(cpFileName),args[0],false);
	else
		ws = gcnew GAMSWorkspace(Path::GetDirectoryName(cpFileName),nullptr,false);

	GAMSJob^ j1 = ws->AddJobFromString(GetBaseModelText(),nullptr,nullptr);
...
int main(array<String ^> ^args)
{
	...
	CreateSaveRestart(Path::Combine(wDir, "tbase"), args);
	...
	Dictionary<String^, double>^ capacity = gcnew Dictionary<String^, double>(); 
	...
	GAMSWorkspace^ ws;
	if (args->Length > 0) 
		ws = gcnew GAMSWorkspace(wDir,args[0],false);
	else
		ws = gcnew GAMSWorkspace(wDir,nullptr,false);
	...
	for each (String^ p in plants) i->AddRecord(p);

	GAMSSet^ j = db->AddSet("j", 1, "markets");
	for each (String^ m in markets)	j->AddRecord(m);

	GAMSParameter^ a = db->AddParameter("a", 1, "capacity of plant i in cases");
	for each (String^ p in plants) a->AddRecord(p)->Value = capacity[p];

	GAMSParameter^ b = db ->AddParameter("b", 1, "demand at market j in cases");
	for each (String^ m in markets) b->AddRecord(m)->Value = demand[m];
	...
	GAMSCheckpoint^ cpBase = ws->AddCheckpoint("tbase");
	{
		GAMSOptions^ opt = ws->AddOptions(nullptr);
		{
			GAMSJob^ t4 = ws->AddJobFromString(GetModelText(), cpBase, nullptr);
