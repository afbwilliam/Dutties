...
procedure ReportGDXError;
var
   S: ShortString;
begin
WriteLn('**** Fatal GDX Error');
GDXobj.GDXErrorStr(GDXobj.GDXGetLastError,S);
WriteLn('**** ',S);
Halt(1);
end;
...
Indx[1] := s;
Values[vallevel] := V;
GDXobj.GDXDataWriteStr(Indx,Values);
end;
...
GDXobj := TGXFILE.CreateD(sysdir,Msg);
if Msg <> ''
then
   begin
...
then
   begin
   //write demand data
   GDXobj.GDXOpenWrite('demanddata.gdx','example1', ErrNr);
   if ErrNr <> 0
   then
      ReportIOError(ErrNr);

   if GDXobj.GDXDataWriteStrStart('Demand','Demand data',1,gms_dt_par,0) = 0
   then
      ReportGDXError;
   WriteData('New-York',324.0);
   WriteData('Chicago' ,299.0);
   WriteData('Topeka'  ,274.0);
   if GDXobj.GDXDataWriteDone = 0
   then
      ReportGDXError;
   ...
   GDXobj.GDXOpenRead(ParamStr(2), ErrNr);
   if ErrNr <> 0
   then
      ReportIOError(ErrNr);

   GDXobj.GDXFileVersion(Msg,Producer);
   WriteLn('GDX file written using version: ',Msg);
   WriteLn('GDX file written by: ',Producer);

   if GDXobj.GDXFindSymbol('x',VarNr) = 0
   then
      begin
   ...
   GDXobj.GDXSymbolInfo(VarNr,VarName,Dim,VarTyp);
   ...
   if GDXobj.GDXDataReadStrStart(VarNr,NrRecs) = 0
   then
      ReportGDXError;

   WriteLn('Variable X has ',NrRecs,' records');
   while GDXobj.GDXDataReadStr(Indx,Values,N) <> 0
   do begin
   ...
   GDXobj.GDXDataReadDone;
   end;

ErrNr := GDXobj.GDXClose;
if ErrNr <> 0
then
   ReportIOError(ErrNr);
