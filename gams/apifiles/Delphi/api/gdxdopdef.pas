unit gdxdopdef; { Delphi object wrapper generated by apiwrapper for GAMS Version 24.0.2 }
...
function gdxGetReady(var Msg: ShortString): boolean;
begin
Result := gdxdpdef.gdxGetReady(Msg);
end;

function  gdxGetReadyX(var Msg: ShortString): boolean;
begin
Result := gdxdpdef.gdxGetReadyX(Msg);
end;

function  gdxGetReadyD(const Dir: ShortString; var Msg: ShortString): boolean;
begin
Result := gdxdpdef.gdxGetReadyD(Dir,Msg);
end;

function  gdxGetReadyL(const LibName: ShortString; var Msg: ShortString): boolean;
begin
Result := gdxdpdef.gdxGetReadyL(LibName,Msg);
end;

procedure gdxSetLoadPath(const s: shortString);
begin
gdxdpdef.gdxSetLoadPath(s);
end;

procedure gdxGetLoadPath(var s: shortString);
begin
gdxdpdef.gdxGetLoadPath(s);
end;
...
inherited Create;
own := true;
if not gdxdpdef.gdxCreate(pgdx, Msg) then begin
   pgdx := nil;
   if Msg = '' then
...
inherited Create;
own := true;
if not gdxdpdef.gdxCreateX(pgdx, Msg) then begin
   pgdx := nil;
   if Msg = '' then
...
inherited Create;
own := true;
if not gdxdpdef.gdxCreateD(pgdx, Dir, Msg) then begin
   pgdx := nil;
   if Msg = '' then
...
inherited Create;
own := true;
if not gdxdpdef.gdxCreateL(pgdx, LibName, Msg) then begin
   pgdx := nil;
   if Msg = '' then
...
own := false;
pgdx := p;
if not gdxdpdef.gdxGetReady(Msg) then begin
   pgdx := nil;
   if Msg = '' then
...
begin
result := gdxdpdef.gdxAcronymAdd(pgdx, AName, Txt, AIndx);
end;

function  TGXFile.gdxAcronymCount: Integer;
begin
result := gdxdpdef.gdxAcronymCount(pgdx);
end;
...
begin
result := gdxdpdef.gdxAcronymGetInfo(pgdx, N, AName, Txt, AIndx);
end;
...
begin
result := gdxdpdef.gdxDataWriteMapStart(pgdx, SyId, ExplTxt, Dimen, Typ, UserInfo);
end;

function  TGXFile.gdxDataWriteRaw(const KeyInt: TgdxUELIndex; const Values: TgdxValues): Integer;
begin
result := gdxdpdef.gdxDataWriteRaw(pgdx, KeyInt, Values);
end;
...
begin
result := gdxdpdef.gdxDataWriteRawStart(pgdx, SyId, ExplTxt, Dimen, Typ, UserInfo);
end;

function  TGXFile.gdxDataWriteStr(const KeyStr: TgdxStrIndex; const Values: TgdxValues): Integer;
begin
result := gdxdpdef.gdxDataWriteStr(pgdx, KeyStr, Values);
end;
