...
Module Transport5
    ...
    Sub Main()
        ...
        For Each b As Double In bmultlist
            Dim t5 As GAMSJob = ws.AddJobFromString("bmult=" + b.ToString + "; solve transport min z us lp; ms=transport.modelstat; ss=transport.solvestat;", cp)
            t5.Run()
            Console.WriteLine("Scenario bmult= {0}", b, ":")
            Console.WriteLine("  Modelstatus:  {0}", t5.OutDB.GetParameter("ms").FindRecord().Value)
            Console.WriteLine("  Solvestatus:  {0}", t5.OutDB.GetParameter("ss").FindRecord().Value)
            Console.WriteLine("  Obj: {0}", t5.OutDB.GetVariable("z").FindRecord().Level)
        Next
    End Sub
