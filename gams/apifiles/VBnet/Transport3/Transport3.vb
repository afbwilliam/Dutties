...
Module Transport3
    ...
    Sub Main()
        ...
        Dim t3 As GAMSJob = ws.AddJobFromString(GetDataText())
        t3.Run()
        ...
        Using opt As GAMSOptions = ws.AddOptions()
            ...
            t3.Run(opt)
            For Each rec As GAMSVariableRecord In t3.OutDB.GetVariable("x")
        ...
        Using opt As GAMSOptions = ws.AddOptions()
            ...
            Dim t3a As GAMSJob = ws.AddJobFromString(GetDataText())
            Dim t3b As GAMSJob = ws.AddJobFromString(GetModelText())
            t3a.Run()
            opt.Defines.Add("gdxincname", t3a.OutDB.Name)
            opt.AllModelTypes = "xpress"
            t3b.Run(opt, t3a.OutDB)
            For Each rec As GAMSVariableRecord In t3b.OutDB.GetVariable("x")
                Console.WriteLine("x(" + rec.Keys(0) + "," + rec.Keys(1) + "): level=" + rec.Level.ToString + " marginal=" + rec.Marginal.ToString)
            Next
        End Using
    End Sub
