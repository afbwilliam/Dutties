...
public class Transport12 {
     ...
     public static void main(String[] args) {
            ...
            GAMSParameter bmult = db1.addParameter("bmultlist", 1, "");
            GAMSParameter zscen1 = db1.addParameter("zscen", 1, "");
            ...
            GUSSCall(dict, mi, "transport us lp min z", null, null, null); //System.out);

            for (GAMSParameterRecord rec : db1.getParameter(zscen1.getName()))
               System.out.println(rec.getKeys()[0] + " obj: " + rec.getValue());
            ...
            for (int j = 0; j < 4; j++) {
                for (GAMSSetRecord irec : t12.OutDB().getSet("i"))
                    for (GAMSSetRecord jrec : t12.OutDB().getSet("j")) {
                        String[] keys = new String[] { "s" + j, irec.getKeys()[0], jrec.getKeys()[0] };	
            ...
            GUSSCall(dict2, mi2, "transport us lp min z", null, null, System.out);
     ...
     static void GUSSCall(GAMSSet dict, GAMSModelInstance mi, String solveStatement, GAMSOptions opt, GAMSModelInstanceOpt miOpt, PrintStream output) {
         ...
         String scenName = dict.getFirstRecord(new String[] { " ", "scenario", " " }).getKeys()[0];
         GAMSSet scenSymbol = dict.getDatabase().getSet(scenName);

         for (GAMSSetRecord rec : dict)
         {
             if (rec.getKeys()[1].toLowerCase().equals("scenario"))
                 continue;
             if (rec.getKeys()[1].toLowerCase().equals("param")) 
             {
                 int modifierDim = dict.getDatabase().getParameter(rec.getKeys()[2]).getDimension() - scenSymbol.getDimension();
                 if (modifierDim < 0)
                     throw new GAMSException("Dimension of " + rec.getKeys()[2] + " too small");
                 GAMSModifier mod = new GAMSModifier(mi.SyncDB().addParameter(rec.getKeys()[0], modifierDim, ""));
                 GAMSParameter param = dict.getDatabase().getParameter( rec.getKeys()[2]);
                 modifierList.add( 
                     new Object[] { mod , param } 
                 );
             }
             else  if ((rec.getKeys()[1].toLowerCase().equals("lower")) || 
                      (rec.getKeys()[1].toLowerCase().equals("upper")) || 
                      (rec.getKeys()[1].toLowerCase().equals("fixed"))) 
                   {
                       int modifierDim = (int) (dict.getDatabase().getParameter(rec.getKeys()[2]).getDimension() - scenSymbol.getDimension());
                       if (modifierDim < 0)
                          throw new GAMSException("Dimension of " + rec.getKeys()[2] + " too small");
                       GAMSVariable modifierVar = null;
                       try  {
                          modifierVar = dict.getDatabase().getVariable(rec.getKeys()[0]);
                       }
                       catch (Exception e) {
                           modifierVar = mi.SyncDB().addVariable(rec.getKeys()[0],modifierDim, GAMSGlobals.VarType.FREE, "");
                       }
                       if (rec.getKeys()[1].toLowerCase() == "lower") 
                       {
                           GAMSModifier mod = new GAMSModifier(modifierVar, GAMSGlobals.UpdateAction.LOWER, mi.SyncDB().addParameter(rec.getKeys()[2], modifierDim, ""));
                           GAMSParameter param = dict.getDatabase().getParameter(rec.getKeys()[2]);
                           modifierList.add(
                               new Object[] { mod , param } 
                           );
                       }
                       else if (rec.getKeys()[1].toLowerCase().equals("upper")) 
                            {
                              GAMSModifier mod = new GAMSModifier(modifierVar, GAMSGlobals.UpdateAction.UPPER, mi.SyncDB().addParameter(rec.getKeys()[2], modifierDim, ""));
                              GAMSParameter param = dict.getDatabase().getParameter(rec.getKeys()[2]);
                              modifierList.add(
                                 new Object[] { mod , param } 
                              );
                                    ...
                                    GAMSModifier mod = new GAMSModifier(modifierVar, GAMSGlobals.UpdateAction.FIXED, mi.SyncDB().addParameter(rec.getKeys()[2], modifierDim, ""));
                                    GAMSParameter param = dict.getDatabase().getParameter(rec.getKeys()[2]);
                                    modifierList.add(
                                       new Object[] { mod , param } 
                                    );
                   ...
                   else if ((rec.getKeys()[1].toLowerCase().equals("level")) || (rec.getKeys()[1].toLowerCase().equals( "marginal")))
                        {
                           // Check that parameter exists in GAMSDatabase, will throw an exception if not
                           @SuppressWarnings("unused")
                           GAMSParameter x = dict.getDatabase().getParameter(rec.getKeys()[2]);
                        }
                        else
                            throw new GAMSException("Cannot handle UpdateAction " + rec.getKeys()[1]);
         }
         ...
         for (GAMSSetRecord s : scenSymbol)
         {
             for (Object[] tuple : modifierList)
             {
                 ...
                 for (int i = 0; i < p.getDimension(); i++)
                     myKeys[i] = rec.getKeys()[scenSymbol.getDimension()+i];
                 p.addRecord(myKeys).setValue( rec.getValue() );
                   
                 while (pscen.hasNext()) {
                    rec = pscen.next();
                    myKeys = new String[p.getDimension()];
                    for (int i = 0; i < p.getDimension(); i++)
                        myKeys[i] = rec.getKeys()[scenSymbol.getDimension()+i];
                    p.addRecord(myKeys).setValue( rec.getValue() );
                 }
             ...
             if (outList.size() == 0)
                 for (GAMSSetRecord rec : dict)
                 {
                     if ((rec.getKeys()[1].toLowerCase().equals("level")) || (rec.getKeys()[1].toLowerCase().equals("marginal"))) 
                     {
                         GAMSSymbol<?> sym = mi.SyncDB().getSymbol(rec.getKeys()[0]);
                         GAMSParameter param = dict.getDatabase().getParameter(rec.getKeys()[2]);
                         String str = rec.getKeys()[1].toLowerCase();
                         outList.add(
                 ...
                 if ((str.equals("level")) && (symbol instanceof GAMSVariable)) 
                 {
                     ...
                     for (GAMSVariableRecord rec : var)
                     {
                         for (int i = 0; i < rec.getKeys().length; i++)
                             myKeys[scenSymbol.getDimension() + i] = s.getKeys()[i];
