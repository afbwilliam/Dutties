...
JNIEXPORT jboolean JNICALL Java_com_gams_api_cfgm_AlgCreate(JNIEnv *env, jobject obj, jint alg, jlongArray psl, jstring sysDir, jobjectArray msg)
{
   ...
   if (fid == NULL) return 0;
   pcfg.i = (*env)->GetLongField(env, obj, fid);
   local_psl.i = *((*env)->GetLongArrayElements(env, psl, NULL));
   local_sysDir = (char *)(*env)->GetStringUTFChars(env, sysDir, NULL);
   buffer_msg[0] = '\0';
   rc_cfgAlgCreate = cfgAlgCreate((cfgHandle_t)pcfg.p, alg, &(local_psl.p), local_sysDir, buffer_msg);
   (*env)->ReleaseLongArrayElements(env, psl, &(local_psl.i), JNI_COMMIT);
...
JNIEXPORT void JNICALL Java_com_gams_api_cfgm_AlgFree(JNIEnv *env, jobject obj, jint alg, jlongArray vpsl)
{
   ...
   if (fid == NULL) return ;
   pcfg.i = (*env)->GetLongField(env, obj, fid);
   local_vpsl.i = *((*env)->GetLongArrayElements(env, vpsl, NULL));
   cfgAlgFree((cfgHandle_t)pcfg.p, alg, &(local_vpsl.p));
   (*env)->ReleaseLongArrayElements(env, vpsl, &(local_vpsl.i), JNI_COMMIT);
}
