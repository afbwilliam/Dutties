*Illustrate how explicit set helps correct problem with using * as set indicator in input data
...
$Offtext
 ...
 Equations  cc(t)    capacity constarint
            ...
            pd2       profit definition ;
 cc(t)..     sum(p, x(p,t)) =l= m;
 sb(r,tt+1).. s(r,tt+1) =e= s(r,tt) - sum(p, a(r,p)*x(p,tt));
 pd2.. profit =e= sum(t, sum(p, c(p,t)*x(p,t))-sum(r, misc("storage-c",r)*s(r,t)))
...
Featured item 1 Set designator
Featured item 2 *
Featured item 3 Universal set
Featured item 4 Problems
include robert2.gms
Description
