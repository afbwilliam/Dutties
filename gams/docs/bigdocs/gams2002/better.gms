...
*slightly reformatted from model library
$Ontext
...
Table  miscdata(inputitem,rawmateral)  miscelaneous production data
 ...
 capacity(quarter)..
        sum(products, production(products,quarter)) =l= mxcapacity;
 stockbalan(rawmateral,Quarters+1)..
      openstock(rawmateral,Quarters+1) =e=
      openstock(rawmateral,Quarters)
      - sum(products, usage(rawmateral,products)
                   *production(products,Quarters));
 profitacct.. profit =e=
   sum(quarter,
...
Featured item 1 Long names
Featured item 2 Typing
Featured item 3 Text formatting
Description
Illustration of improved model file format
