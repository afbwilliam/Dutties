...
$stitle
  parameter
      ...
      rations(r)  lvstk rations feeding alternative (cr per head)/ rec-1  5.141 , rec-2  21.646 , rec-3  49.845/
...
* land req for lvstk seems high but ok - pg 65 para 3
 table lio(s,r) land requirements for lvstk feed alternatives (ha per head)
 ...
 table labor(p,tm)  labor requirements for cropping (mandays per ha)
 ...
 table llab(tm,r) labor requirement for lvstk feed  (mandays per head)
...
* family labor available: correct?
 scalar fwage     family reservation wage rate (cr per man-month) /     75 /
        ...
        vetpr     cost of vetinanry servic          (cr per head) /     1    /
...
* some of these yields look odd: medium land has higher land than good
 ...
 table techc(p,km) cropping technology rquirements (cr per ha)
...
$stitle model definition
 ...
 equations landb(s)     land balance
           ...
           avet         accounting: vetinary costs                                   (cr)
 ...
 option lp=osl;
 FILE  OPT   osl option file  / osl.OPT /;
 PUT OPT;
 PUT 'objrng xcrop,xlive,sales'/
     'rhsrng labc,landb '/;
 PUTCLOSE OPT;
 agreste.optfile=1;
 agreste.dictfile=4;
...
Featured item 3
Featured item 4
include (gamsbas.bas,gamsbas.gms,rgamsbas.bat)
Description
illustrate ranging analysis for selected components
